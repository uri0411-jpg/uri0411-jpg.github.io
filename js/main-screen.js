...
import { startCountdown, stopCountdown } from './main-screen/rating.js';
import { buildScoreExplainer } from './main-screen/explainer.js';
import { buildScoreSparkline, buildParamBar, buildTwilightRow } from './main-screen/charts.js';
import {
  startSkyGradient,
  stopSkyGradient,
  markScreenOpened,
  computeDaySkyColors,
  repaintScoreColors as _repaintSkyScoreColors,
} from './main-screen/sky-renderer.js';
...
// Live sky rendering (gradient loop, score-color repaint, day sky colors)
// → js/main-screen/sky-renderer.js

/**
 * Re-export of sky-renderer's repaintScoreColors that fills in the
 * parent's _spotAvgScores (which is set via setSpotAvgScores). Preserves
 * the no-arg API used by [app.js].
 */
export function repaintScoreColors() {
  return _repaintSkyScoreColors(_spotAvgScores);
}
...
export async function initMainScreen(loc, city, weekData, spotAvgScores = null) {
  ...
  if (today) {
    ...
    startSkyGradient(today, loc, locGen, () => _spotAvgScores);
...
export function refreshMainScores(weekData, spotAvgScores = null) {
  ...
  startSkyGradient(today, getState().loc, getState().locGen, () => _spotAvgScores);
  registerCleanup('main', stopSkyGradient);
...
// ─────────────────────────────────────────
//  Countdown timer + multi-event rating UI → js/main-screen/rating.js
// ─────────────────────────────────────────

// ─────────────────────────────────────────
//  Score sparkline / param bars / twilight row → js/main-screen/charts.js
//  Tier-2 score explainer tray → js/main-screen/explainer.js
//  (renderCloudChart removed — was dead code, never called)
// ─────────────────────────────────────────
